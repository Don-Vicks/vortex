use crate::{LifecycleEvent, TxStatus};
use chrono::Utc;
...
pub fn mark_finalized(event: &mut LifecycleEvent, slot: u64) {
    ...
    event.latency_to_finalized_ms = Some((now - event.submitted_at).num_milliseconds());
    event.status = TxStatus::Finalized;
}

pub fn mark_failed(event: &mut LifecycleEvent, failure: crate::FailureInfo) {
