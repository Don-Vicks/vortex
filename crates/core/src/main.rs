...
async fn main() -> Result<()> {
    ...
    let flag_clone_for_signal = Arc::clone(&shutdown_flag);
    let notify_clone_for_signal = Arc::clone(&shutdown_notify);
    tokio::spawn(async move {
        tokio::signal::ctrl_c()
            .await
            .expect("Failed to listen for Ctrl+C");
        tracing::info!("Shutdown signal received. Finishing current cycle...");
        flag_clone_for_signal.store(true, Ordering::Relaxed);
        notify_clone_for_signal.notify_one();
    });
    ...
    let run_daemon_mode = env::var("RUN_DAEMON").unwrap_or_else(|_| "false".to_string()) == "true";

    if run_daemon_mode {
        tracing::info!("Running in Daemon Mode (autonomous background loop)");
        let rpc_client_arc = Arc::clone(&rpc_client);
        // The daemon takes ownership of some items, so we clone what we need
        run_daemon(
        ...
        tracing::info!("Starting Kora Relayer Server on http://0.0.0.0:3000 (Mode: API)");
        let app = Router::new()
...
async fn relay_handler(
    ...
    let tip_stats = match vortex::jito::tip::fetch_tip_stats(&state.rpc_client.url()).await {
        ...
        Err(e) => {
            return Json(RelayResponse {
                ...
                signature: None,
                bundle_id: None,
                error: Some(format!("Tip stats error: {}", e)),
            });
        }
    };
    ...
    // 3. AI Agent Decision
    let tip_decision =
        match vortex::agent::decisions::decide_tip(&state.agent_config, &network_state).await {
...
async fn run_daemon(
    ...
    leader_tracker: vortex::jito::leader::LeaderTracker,
    rpc_url: String,
    log_path: String,
    agent_config: vortex::agent::AgentConfig,
    keypair: Arc<solana_sdk::signature::Keypair>,
    ...
    loop {
        ...
        let (bundle_id, signature, status, failure) = match bundle_result {
            Ok(res) => {
                ...
                (res.bundle_id, res.signature, TxStatus::Pending, None)
            }
            Err(e) => {
        ...
        if matches!(status, TxStatus::Failed) {
            if let Some(f) = failure {
                ...
                if let Ok(analysis) =
                    vortex::agent::decisions::analyze_failure(&agent_config, &f.raw_error).await
                {
                    ...
                    if analysis.action == "refresh_blockhash" || analysis.action == "increase_tip" {
                        let new_tip =
                            (event.tip_lamports as f64 * analysis.suggested_tip_multiplier) as u64;
                        let fresh_blockhash =
                            rpc_client.get_latest_blockhash().await.unwrap_or_default();

                        tracing::info!(
                            "Agent autonomously resubmitting with fresh blockhash and tip {}",
                            new_tip
                        );
                        ...
                        retry_event.tip_reasoning = format!(
                            "Autonomous recovery. Action: {}, Cause: {}",
                            analysis.action, analysis.cause
                        );
                        ...
                        match retry_res {
                            ...
                            Err(e) => {
                                ...
                                let error_type =
                                    vortex::failures::classifier::classify(&format!("{:?}", e));
